...
MainWindow::MainWindow(char *argv[], QWidget *parent) : QWidget(parent)
{
    ...
    buttonAction();   // 按钮操作检测
    // 打开设备文件
    fd = open(QString(argv[1]).toStdString().c_str(), O_RDONLY);
    if (fd < 0)
        qFatal("无法打开输入设备");
    notifier = new QSocketNotifier(fd, QSocketNotifier::Read, this);
    connect(notifier, SIGNAL(activated(int)), this, SLOT(joyCtlVector()));
}
...
void MainWindow::joyCtlVector()
{
    ...
    while (1)
    {
        ...
        // C
        if ((ev.type == 1) && (ev.code == 301) && (ev.value == 1))
        {
            pause.click();
        }
        // D
        if ((ev.type == 1) && (ev.code == 300) && (ev.value == 1))
        {
            goon.click();
        }
        // 退出条件
        if ((l_count % 2 == 0) || (r_count % 2 == 0))
            break;
    }
}
...
void MainWindow::initStartScene()
{
    ...
    title->move(B_MID_POSW - 135, B_MID_POSH - 300);
...
void MainWindow::initButton()
{
    ...
    ss_b3_bk.move(B_MID_POSW, B_MID_POSH + 250);
    ss_b3_bk.hide();

    // 返回按钮
    bk.setParent(this);
    bk.resize(110, 55);
    bk.move(GAME_WIDTH - 125, 10);
...
void MainWindow::paintEvent(QPaintEvent *)
{
    ...
    if (startFlag == 1)
    {
        ...
        painter.drawPixmap(100 - heroHPDisp[toDisplay[2]].m_width / 6 - heroHPDisp[toDisplay[1]].m_width / 6 - 6,
                           GAME_HEIGHT - 50,
