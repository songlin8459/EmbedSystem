...
void get_frame(struct v4l2_dev *dev, int skip_frame)
{
    ...
    for (int i = 0;i < skip_frame; ++i) {
        ...
        printf("image: sequence = %d, timestamp = %lu, datalen = %d\n", buf.sequence, dev->timestamp, dev->data_len);
...
int cam_mipi_on()
{
    ...
    while (1)
	{
        ...
        memcpy(jpg->jpg_buf, camdev->out_data, 460800/20);
        jpg->jpg_size = camdev->data_len/20;
        // save_picture(name, jpg->jpg_buf, jpg->jpg_size, 1);
        // system("./nv12_to_jpg");
        pthread_mutex_unlock(&cam_mutex);
#if 0
        convert_yuv_to_rgb(camdev->out_data, jpg->jpg_buf, width, height, 24);
        pthread_mutex_lock(&cam_mutex_m);
        memset(jpg, 0, sizeof(jpg));
        jpg->jpg_size = convert_rgb_to_jpg_work(jpg->jpg_buf, jpg->jpg_buf, width, height, 24, 80);
        pthread_mutex_unlock(&cam_mutex_m);
#endif
}
