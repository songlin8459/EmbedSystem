...
{
    //获取权限
    QStringList permissions = {"android.permission.READ_EXTERNAL_STORAGE", "android.permission.ACCESS_CHECKIN_PROPERTIES"};
    QtAndroid::PermissionResultMap result = QtAndroid::requestPermissionsSync(permissions, -1);
    if(result["android.permission.READ_EXTERNAL_STORAGE"] == QtAndroid::PermissionResult::Denied)
    {
        //请求权限失败
        qDebug() << "request failed";
    }
    else if(result["android.permission.READ_EXTERNAL_STORAGE"] == QtAndroid::PermissionResult::Granted)
    {
        //请求权限成功
    ...
    if (file.is_open())
    {
        while (std::getline(file, line))
        {
            ...
            while (foundHZY)
            {
                size_t startPos = line.find("event");
                if (startPos != std::string::npos)
                {
                    size_t endPos = line.find(" ", startPos);
                    if (endPos != std::string::npos)
                    {
                        eventStr = line.substr(startPos, endPos - startPos);
                    } else {
                        eventStr = line.substr(startPos);
                    }
                }
                break;
            }
            if (line.find("HZY") != std::string::npos)
            {
                qDebug() << QString(QString::fromStdString(line));
                foundHZY = true;
            }
        }
        file.close();
    ...
    {
        qDebug() << "open /proc/bus/input/devices failed";
    }

    qDebug() << "eventStr: " << QString(QString::fromStdString(eventStr));

    // 读取设备
    loop_counter = 0;
    eventStr = "/dev/input/" + eventStr;
    fd = open(eventStr.c_str(), O_RDONLY);
    if (fd < 0)qFatal("open device failed");
    notifier = new QSocketNotifier(fd, QSocketNotifier::Read, this);
    connect(notifier, SIGNAL(activated(int)), this, SLOT(joyCtlVector_handle()));
}
void MainWindow::joyCtlVector_handle()
{
    QtConcurrent::run(this, &MainWindow::joyCtlVector);
}
void MainWindow::joyCtlVector()
{
    ...
    while (1)
    {
        ...
        // 摇杆控制
        //up
        if((ev.type == 3) && (ev.code == 0) && (ev.value >= 35000))
        {
            heroVector.keyMoveState[0] = 1;
        }
        //down
        if((ev.type == 3) && (ev.code == 0) && (ev.value <= 30000))

            heroVector.keyMoveState[2] = 1;
        if((ev.type == 3) && (ev.code == 0) && (ev.value > 30000) && ev.value < 35000)
        {
            heroVector.keyMoveState[0] = 0;
            heroVector.keyMoveState[2] = 0;
        }
        //right
        if((ev.type == 3) && (ev.code == 1) && (ev.value >= 35000))
            heroVector.keyMoveState[3] = 1;
        //left
        if((ev.type == 3) && (ev.code == 1) && (ev.value <= 30000))
            heroVector.keyMoveState[1] = 1;
        if((ev.type == 3) && (ev.code == 1) && (ev.value > 30000) && ev.value < 35000)
        {
        ...
        //speed_up && shoot
        if((ev.type == 3) && (ev.code == 2) && (ev.value >= 35000))
            heroVector.keyMoveState[4] = 1;
        if((ev.type == 3) && (ev.code == 2) && (ev.value <= 30000))
            heroVector.m_shootState = 1;
        if((ev.type == 3) && (ev.code == 2) && (ev.value > 30000) && (ev.value < 35000))
        {
            heroVector.m_shootState = 0;
            heroVector.keyMoveState[4] = 0;
        }
        if ((ev.type == 3) && (ev.code == 3) && (ev.value >= 2200))
            heroVector.m_shootState = 1;
        if ((ev.type == 3) && (ev.code == 3) && (ev.value <= 1800))
            heroVector.keyMoveState[4] = 1;
        if ((ev.type == 3) && (ev.code == 3) && (ev.value > 1800) && (ev.value < 2200))
        {
            heroVector.m_shootState = 0;
            heroVector.keyMoveState[4] = 0;
        }
        ...
        // 按键控制
        // up
        if ((ev.type == 1) && (ev.code == 0x121) && (ev.value == 1))
        {
            heroVector.keyMoveState[0] = 1;
        }
        if ((ev.type == 1) && (ev.code == 0x121) && (ev.value == 0))
        {
            heroVector.keyMoveState[0] = 0;
        }
        // right
        if ((ev.type == 1) && (ev.code == 0x123) && (ev.value == 1))
        {
            heroVector.keyMoveState[1] = 1;
        }
        if ((ev.type == 1) && (ev.code == 0x123) && (ev.value == 0))
        {
            heroVector.keyMoveState[1] = 0;
        }
        // down
        if ((ev.type == 1) && (ev.code == 0x124) && (ev.value == 1))
        {
            heroVector.keyMoveState[2] = 1;
        }
        if ((ev.type == 1) && (ev.code == 0x124) && (ev.value == 0))
        {
            heroVector.keyMoveState[2] = 0;
        }
        // left
        if ((ev.type == 1) && (ev.code == 0x122) && (ev.value == 1))
        {
            heroVector.keyMoveState[3] = 1;
        }
        if ((ev.type == 1) && (ev.code == 0x122) && (ev.value == 0))
        {
            heroVector.keyMoveState[3] = 0;
        }
        // A
        if ((ev.type == 1) && (ev.code == 0x12a) && (ev.value == 1))
        {
            heroVector.m_shootState = 1;
        }
        if ((ev.type == 1) && (ev.code == 0x12a) && (ev.value == 0))
        {
            heroVector.m_shootState = 0;
        }
        // B
        if ((ev.type == 1) && (ev.code == 0x129) && (ev.value == 1))
        {
            heroVector.m_shootState = 1;
        }
        if ((ev.type == 1) && (ev.code == 0x129) && (ev.value == 0))
        {
            heroVector.m_shootState = 0;
        }
        // Y
        if ((ev.type == 1) && (ev.code == 0x127) && (ev.value == 1))
        {
            heroVector.keyMoveState[4] = 1;
        }
        if ((ev.type == 1) && (ev.code == 0x127) && (ev.value == 0))
        {
            heroVector.keyMoveState[4] = 0;
        }
        // X
        if ((ev.type == 1) && (ev.code == 0x128) && (ev.value == 1))
        {
            heroVector.keyMoveState[4] = 1;
        }
        if ((ev.type == 1) && (ev.code == 0x128) && (ev.value == 0))
        {
            heroVector.keyMoveState[4] = 0;
        }
        // C
        if ((ev.type == 1) && (ev.code == 0x126) && (ev.value == 1))
        {
            flag_stop = 1;
            emit flag_stop_signal(flag_stop);
        }
        // D
        if ((ev.type == 1) && (ev.code == 0x12b) && (ev.value == 1))
        {
            flag_stop = 1;
            emit flag_stop_signal(flag_stop);
        }
        // E
        if ((ev.type == 1) && (ev.code == 0x120) && (ev.value == 1))
        {
            flag_stop = 1;
            emit flag_stop_signal(flag_stop);
        }
        // F
        if ((ev.type == 1) && (ev.code == 0x125) && (ev.value == 1))
        {
            flag_stop = 1;
            emit flag_stop_signal(flag_stop);
        }
        //qDebug() << ev.type << ev.code << ev.value;
        loop_counter++;
        if(loop_counter%4 == 0)
        {
            loop_counter = 1;
...
void MainWindow::initStartScene()
{
    ...
    pal.setBrush(QPalette::Background,
                 QBrush(img.scaled(GAME_WIDTH, GAME_HEIGHT,
    ...
    title->move(GAME_WIDTH/2 - 573, 350);

    /***** 键位介绍文本设置 *****/
    // intro->setText("左摇杆：控制移动"
    //                "\n\n"
    //                "右摇杆向左：快速机动/加速: Shift"
    //                "\n\n"
    //                "右摇杆向右：射击"
    //                "\n\n\n"
    //                "松开射击键一段时间后会自动补充弹药");
    intro->setPixmap(QPixmap(KB));
    intro->setAlignment(Qt::AlignHCenter);
    intro->move(GAME_WIDTH/2 - 700, 200);
    intro->resize(1400, 1200);
...
void MainWindow::initButton()
{
    ...
    ss_b1.move(B_MID_POSW, B_MID_POSH + 100);

    ss_b2.setParent(this);
    ss_b2.setText(QString("退出"));
    ss_b2.move(B_MID_POSW, B_MID_POSH + 2 * (BUTTON_H + 50) + 100);

    // 键位介绍相关
    ss_b3.setParent(this);
    ss_b3.setText(QString("键位介绍"));
    ss_b3.move(B_MID_POSW, B_MID_POSH + BUTTON_H + 50 + 100);

    ss_b3_bk.setParent(this);
    ss_b3_bk.setText(QString("返回"));
    ss_b3_bk.move(B_MID_POSW, GAME_HEIGHT- 300);
    ss_b3_bk.hide();

    // 游戏中退出按钮
    bk.setParent(this);
    bk.resize(180, 70);
    bk.move(GAME_WIDTH - 195, 10);
    bk.setText(QString("退出"));
    bk.hide();
    ...
    diff_Easy.move(B_MID_POSW, B_MID_POSH - 200);
    ...
    diff_Hard.move(B_MID_POSW, B_MID_POSH + 200);
...
void MainWindow::buttonAction()
{
    ...
    // 点击退出游戏
    connect(&ss_b2, &QPushButton::released, [=]()
    { close(); });
            ...
            [=]()
    {
        ...
        int ret = QMessageBox::question(this, "退出",
                                        "你确定要退出吗？",
    ...
    connect(this, &MainWindow::flag_stop_signal, [=](bool flag_stop)
    {
        if(flag_stop)
        {
            if(!pause.isHidden())pause.click();
        }
    });
    // 暂停按钮
...
void MainWindow::paintEvent(QPaintEvent *)
{
    ...
    if (startFlag == 1)
    {
        ...
        // 绘制英雄血量
        painter.drawPixmap(6, GAME_HEIGHT - 153, healthIcon.width() * 1.3,
                           healthIcon.height() * 1.3, healthIcon);

        painter.drawPixmap(160,
                           GAME_HEIGHT - 143,
                           heroHPDisp[toDisplay[2]].m_width / 4,
                heroHPDisp[toDisplay[2]].m_height / 4,
                heroHPDisp[toDisplay[2]].numImg);

        painter.drawPixmap(160 - heroHPDisp[toDisplay[2]].m_width / 4 - 3,
                GAME_HEIGHT - 143,
                heroHPDisp[toDisplay[1]].m_width / 4,
                heroHPDisp[toDisplay[1]].m_height / 4,
                heroHPDisp[toDisplay[1]].numImg);

        painter.drawPixmap(160 - heroHPDisp[toDisplay[2]].m_width / 4 - heroHPDisp[toDisplay[1]].m_width / 4 - 4,
                GAME_HEIGHT - 143,
                heroHPDisp[toDisplay[0]].m_width / 4,
                heroHPDisp[toDisplay[0]].m_height / 4,
                heroHPDisp[toDisplay[0]].numImg);

        // 绘制弹匣
        painter.drawPixmap(GAME_WIDTH - magazineBoard.m_bulletNum->width() + 10,
                           GAME_HEIGHT - magazineBoard.m_bulletNum->height() - 90,
                           magazineBoard.m_img.width(),
                           magazineBoard.m_img.height(),
                           magazineBoard.m_img);
    }
}
                    ...
                    for (int k = 0; k < BOMB_NUM; k++)
                    {
                        if (bombs[k].m_state) // 寻找一个空闲的爆炸
                        {
                            ...
                            bombs[k].m_X = enemys[i].m_X +
                                    (enemys[i].m_img.width() - bombs[k].m_imgArray[bombs[k].m_imgIndex].width()) / 2 - 20;
                            bombs[k].m_Y = enemys[i].m_Y +
                                    (enemys[i].m_img.height() - bombs[k].m_imgArray[bombs[k].m_imgIndex].height()) / 2 + 10;
                            break;
                        }
                    }
                }
        ...
        if (enemys2[i].m_rect.intersects(myHero.m_rect) && myHero.m_state == 0)
        {
            ...
            for (int k = 0; k < BOMB2_NUM; k++)
            {
                if (bombs2[k].m_state) // 寻找一个空闲的爆炸
                {
                    ...
                    // 更新爆炸坐标
                    bombs2[k].m_X = enemys2[i].m_X +
                            (enemys2[i].m_img.width() - bombs2[k].m_imgArray[bombs2[k].m_imgIndex].width()) / 2;
                    bombs2[k].m_Y = enemys2[i].m_Y +
                            (enemys2[i].m_img.height() - bombs2[k].m_imgArray[bombs2[k].m_imgIndex].height()) / 2;
                    ;
                    break;
                }
            }
